...
* `docker-compose.ms.yml`: Creates a container just for the server application (persistence mode will be set to MicroStream) and launches it.
...
Open a terminal and execute the command `jmeter -Jsample_variables=rduration -n -t terminals.jmx -l results.jtl` to run the test in JMeter's CLI-mode. This will execute the testplan defined in `terminals.jmx`. The results will be written as CSV data to the `results.jtl` file. The `rduration` variable is added to the `results.jtl` file. This variable contains the *pure* processing time of the request on the server, see the controller classes for further detail. 
